...
import { useGetTourBySlug, useListTours, getGetTourBySlugQueryKey } from "@workspace/api-client-react"
...
export default function TourDetail() {
  ...
  const { data: similarTours = [] } = useListTours(
    ...
    { query: { enabled: !!tour?.destination } }
  )
    ...
    return (
      <Layout>
        <div className="pt-24 pb-12 min-h-screen flex flex-col">
          ...
          <div className="container mx-auto px-4"><div className="h-96 bg-muted animate-pulse rounded-xl" /></div>
        </div>
      </Layout>
    )
  }
    ...
    return (
      <Layout>
        <div className="pt-32 pb-24 text-center min-h-[60vh] flex flex-col items-center justify-center">
          ...
          <Button asChild><Link href="/tours">Back to Tours</Link></Button>
        </div>
      </Layout>
    )
  }
    ...
    <Layout>
      ...
      <div className="relative pt-20 h-[60vh] md:h-[70vh] bg-black">
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                {tour.gallery_urls.map((_, idx) => (
                  <button
                    ...
                    className={`w-2.5 h-2.5 rounded-full transition-all ${idx === activeImageIdx ? 'bg-white scale-125' : 'bg-white/50 hover:bg-white/80'}`}
                  />
                ))}
              </div>
            )}
        ...
        <div className="absolute top-24 left-4 md:left-8 z-20">
          <Link href="/tours" className="inline-flex items-center text-white hover:text-primary transition-colors bg-black/20 backdrop-blur-md px-3 py-1.5 rounded-full text-sm font-medium">
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-10">
            <div className="bg-card p-6 md:p-8 rounded-2xl shadow-lg border">
              <div className="flex flex-wrap gap-2 mb-4">
                <Badge className="bg-primary/20 text-primary hover:bg-primary/30 border-transparent">{tour.activity_type}</Badge>
                {tour.is_featured && <Badge variant="secondary">Featured</Badge>}
              </div>
              <h1 className="text-3xl md:text-5xl font-serif font-bold mb-4 leading-tight">{tour.title}</h1>
            ...
            <div className="space-y-6">
              ...
              <Accordion type="multiple" className="space-y-4">
                {tour.itinerary.map((day, idx) => (
                  <AccordionItem key={idx} value={`day-${day.day}`} className="bg-card border rounded-xl overflow-hidden px-2">
          ...
          <div className="lg:sticky lg:top-24 space-y-6">
            <div className="bg-card border rounded-2xl p-6 shadow-xl">
              <div className="mb-6">
                <p className="text-sm text-muted-foreground font-medium uppercase tracking-wider mb-1">Starting from</p>
                <div className="flex items-end gap-2">
                  <span className="text-4xl font-serif font-bold text-primary">${tour.price_from}</span>
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex justify-between text-sm py-2 border-b">
                  ...
                  <span className="font-medium">{tour.max_group_size ? `Up to ${tour.max_group_size}` : 'Flexible'}</span>
                </div>
              </div>

              <Button size="lg" className="w-full h-14 text-lg rounded-xl mb-4 shadow-lg hover:shadow-xl transition-all" asChild>
            ...
            <div className="bg-muted p-6 rounded-2xl text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">Our safari experts are available to customize this itinerary for you.</p>
              <Button variant="outline" className="w-full rounded-xl bg-transparent border-primary/20 hover:bg-primary/5 text-primary" asChild>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-background/90 backdrop-blur-md border-t z-40">
        <div className="flex items-center justify-between gap-4 max-w-md mx-auto">
          ...
          <Button size="lg" className="flex-1 rounded-full shadow-lg" asChild>
            <Link href={`/booking?tour_id=${tour.id}`}>Book Now</Link>
          </Button>
        </div>
      </div>
    </Layout>
